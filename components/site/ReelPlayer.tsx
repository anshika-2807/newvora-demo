  ...
  if (kind === "video") {
    return (
      <button onClick={() => { const m = !muted; setMuted(m); if (vid.current) vid.current.muted = m; }} className="relative w-full h-full block group" aria-label="Toggle sound">
        ...
        <span className="absolute bottom-3 right-3 h-8 w-8 rounded-full bg-surface/85 grid place-items-center text-ink text-xs">{muted ? <Icon name="mute" className="w-4 h-4" /> : <Icon name="speaker" className="w-4 h-4" />}</span>
