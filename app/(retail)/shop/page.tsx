...
export const metadata = {
  title: "Newvora — Online Store & Wholesale",
  description: "Explore the Newvora demo store — browse categories, search, and check out at retail and wholesale prices.",
};
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-cream to-ivory">
        <div className="max-w-7xl mx-auto px-5 py-14 md:py-20 grid md:grid-cols-2 gap-10 items-center">
          <div className="animate-fadeUp">
            <p className="text-gold-dark tracking-[0.3em] uppercase text-xs mb-4">Newvora · Online Store</p>
            <h1 className="font-display text-5xl md:text-6xl leading-[1.05] text-ink">
              Shop your <span className="text-gold-gradient">favourites</span>, effortlessly.
            </h1>
            <p className="text-muted mt-5 max-w-md leading-relaxed">
              A curated catalogue with fast search, live stock and secure checkout — priced for both retail and wholesale. This is a live demo store you can explore end to end.
            </p>
            ...
            <div className="flex items-center gap-6 mt-8 text-sm text-muted">
              <span>★ 4.8 avg rating</span><span>·</span><span>50,000+ customers</span><span>·</span><span>{products.length} products live</span>
            </div>
          </div>
          <div className="relative h-[360px] md:h-[440px]">
            <div className="absolute right-0 top-0 w-52 h-64 rounded-3xl overflow-hidden shadow-luxe rotate-3 animate-float"><ProductImage name="New In" /></div>
            <div className="absolute left-2 top-16 w-44 h-56 rounded-3xl overflow-hidden shadow-luxe -rotate-6 animate-float" style={{ animationDelay: "1s" }}><ProductImage name="Bestseller" /></div>
            <div className="absolute left-28 bottom-0 w-40 h-48 rounded-3xl overflow-hidden shadow-gold rotate-2 animate-float" style={{ animationDelay: "2s" }}><ProductImage name="Trending" /></div>
      ...
      <section id="bestsellers" className="max-w-7xl mx-auto px-5 py-8">
        <div className="flex items-end justify-between mb-7">
          ...
          <Link href={cats[0] ? `/shop/c/${cats[0].slug}` : "/shop"} className="nav-link text-sm text-emerald">View all →</Link>
        </div>
